...
export default function TrustedBrands() {
  const brands = [
    ...
    { name: "AtoS", logo: "AtoS" },
    { name: "Schmitt-Thompson", logo: "Schmitt+Thompson" },
  ];
  ...
  return (
    <section className="py-4 bg-white">
      <div className="max-w-7xl mx-auto px-6">
        ...
        {/* Brand Logos */}
        <div className="grid grid-cols-2 md:grid-cols-5 gap-8 items-center justify-items-center opacity-40 grayscale">
          {brands.map((brand, index) => (
            <div key={index} className="flex items-center justify-center h-12">
              <span className="text-xl md:text-2xl font-bold text-gray-600 whitespace-nowrap">
                {brand.logo}
              </span>
            </div>
          ))}
